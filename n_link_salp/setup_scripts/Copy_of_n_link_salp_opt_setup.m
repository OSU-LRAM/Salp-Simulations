...
baseframe = 'com-mean';
load(['salp_3_link_basis_15_t_angle_30_baseframe_' baseframe '.mat'])
...
% input_func = {@(t, x) 0, @(t, x) sin(omega*t) + 1, @(t, x) cos(omega*t) + 1};
% InputOptimUtils.getJetOnlyForceFunction(input_func, thrust_angle, n);
% solver_config.local_force_func = InputOptimUtils.getJetOnlyForceFunction(input_func, thrust_angle, n);
solver_config.local_force_func = @(t,x) local_fixed_force(zeros(3*n, 1));% getMinGaitForceFunction2(n, q_init, B_aug, domain, q_dot_des, thrust_angle, omega, max_force_thrust);
% solver_config.local_force_func = getMinGaitForceFunctionFeedback(T, n, q_init, B_aug, domain, q_dot_des, thrust_angle, omega, max_force_thrust, false);
...
anim_config.baseframe = baseframe;
